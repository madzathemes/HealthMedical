...
<div class="row">
	...
	<div class="col-md-<?php if ($mt_layout != "full") { echo "8 "; echo esc_attr($col_8); } else {  echo "12 "; } echo esc_attr($mt_float_layout);  ?> ">
	    ...
	    <?php global $more; $more = 0; while ( have_posts() ) : the_post();  $mt_portfolio_slider_height = get_post_meta(get_the_ID(), "mt_portfolio_slider_heigh", true); ?>
					...
					<div class="entry-page-image entry-page-image-cause">
						<a href="<?php the_permalink(); ?>"><span class="dark-background-2"></span>
							<?php $src = wp_get_attachment_image_src( get_post_thumbnail_id(get_the_ID()), array( 999,999 ), false, '' ); 
						 	echo get_the_post_thumbnail( get_the_ID(), array(960, $mt_portfolio_slider_height, 'bfi_thumb' => true, 'class' => 'cause-img'), $src[0] );
						 	?>
						</a>
					</div>
				<?php endif; ?>	
			...
			<footer class="entry-meta">
				...
				<a href='<?php echo get_post_meta(get_the_ID(), 'mt_causes_button_url', TRUE); ?>' class='more-link mt-donate-link'><span><?php echo get_post_meta(get_the_ID(), 'mt_causes_button_name', TRUE); ?></span></a>
