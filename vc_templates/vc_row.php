...
if($row_type == 'row') {
	$output .= '<div class="mt_style_row mt_bg_fix '.$mt_bg_type.' '.$el_class.'"';
	...
	if($video == "show_video"){
		...
		$output .= '<div class="mobile-video-image " style="background-image: url('.$v_image.')"></div><div class="video-overlay';
								...
								$output .= '></div><div class="video-wrap">
									...
									<video class="video videobcg hidden-sm hidden-xs" width="1920" height="800" poster="'.$v_image.'" style="margin-top:' . ( preg_match( '/(px|em|\%|pt|cm)$/', $video_fix ) ? $video_fix : $video_fix . 'px' ) . '" controls="controls" preload="auto" loop autoplay muted>';
...
} else if($type == 'full_width' or get_post_meta($post->ID, "layout_positions", true)!='full') {
	$output .= '<div class="mt-madza-container"><div class="row">';  
} else {
	$output .= '<div class="container mt-madza-container"><div class="row">'; 
}
//CONTENT
